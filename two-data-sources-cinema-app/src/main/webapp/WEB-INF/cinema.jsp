...
<div class="container">
  ...
  <h1>Cinema</h1>
<a href="/">Home</a> - Go to Home<br/>
  <form class="movie-input-form form-inline" action="cinema"
        method="post">
    <p>Add Movie</p>
  ...
  <c:if test="${count > 0}">
    <c:if test="${page > 1}">
      <a href="<c:url value="cinema"><c:param name="page" value="${page - 1}"/><c:param name="field" value="${field}"/><c:param name="key" value="${key}"/></c:url>">&lt; Prev</a>&nbsp;
    </c:if>
    Showing records ${start} to ${end} of ${count}
    <c:if test="${page < pageCount}">
      &nbsp;<a href="<c:url value="cinema"><c:param name="page" value="${page + 1}"/><c:param name="field" value="${field}"/><c:param name="key"
